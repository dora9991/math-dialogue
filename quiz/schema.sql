...
create table if not exists quiz_study_time (
  ...
  note       text,                            -- 家庭学習で頑張ったこと（任意）
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (account_id, quiz_id, jst_date)
);
alter table quiz_study_time enable row level security;
alter table quiz_study_time add column if not exists note text;
-- ポリシーを一切作らない = anonキーからの直接アクセスは全拒否（RPC経由のみ）

-- 記録（同じ生徒×同じ授業×同じ日は上書き＝押し直しOK）
create or replace function quiz_study_time_submit(p_account_id text, p_quiz_id text, p_minutes int, p_note text default null)
returns jsonb language plpgsql security definer set search_path = public as $$
...
begin
  ...
  insert into quiz_study_time (account_id, quiz_id, jst_date, minutes, note, updated_at)
    values (p_account_id, coalesce(p_quiz_id,'unknown'), v_today, p_minutes, p_note, now())
  on conflict (account_id, quiz_id, jst_date) do update set
    minutes = excluded.minutes, note = excluded.note, updated_at = now();
  return jsonb_build_object('ok', true);
end $$;

-- 教師用：選んだ学校・クラス（＋任意でその授業）の、今日の学習時間まとめ。
-- ベスト3・平均・全員の記録（頑張ったことのメモ含む）をまとめて返す（投影して見せられるように）
...
begin
  ...
  return jsonb_build_object('rows', coalesce((
    select jsonb_agg(jsonb_build_object('account_id', st.account_id, 'quiz_id', st.quiz_id, 'minutes', st.minutes, 'note', st.note) order by st.minutes desc)
